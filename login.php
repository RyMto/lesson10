...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (strlen(trim($_POST["password"] ?? "")) == 0) { //入力されているか確認
        //メール形式確認
        $errors = "メールアドレスとパスワードを正しく入力して下さい。";
    }
    ...
    if (empty($errors)) {
        ...
        //ユーザー情報が取り出せた、かつパスワードが一致すればセッションに値を代入し、マイページ遷移
        if ($user && password_verify($input["password"], $user["password"])) {
            ...
            //ログイン情報保持にチェックがあればセッションにセット
            if ($_POST['login_keep'] == 1) {
                $_SESSION["login_keep"] = $_POST["login_keep"];
            }

            //ログイン情報保持にチェックがあればクッキーをセット、なければ削除する。
...
<body>
    ...
    <form method="POST" action="">
        ...
        <div class="item">
            <label>
            <input type="checkbox" name="login_keep" value="1"<?php if ($_COOKIE['login_keep'] ?? '') {
